class instr_template:
    ...
    def load_qp_template(self):
        self.input_template[
            "instr_comment"
        ] = f"""Predict the magnitude for [Num] in the comment: {{masked}}
magnitude: """

        self.input_template[
            "instr_headline"
        ] = f"""Predict the magnitude for [Num] in the headline: {{masked}}
magnitude: """

        self.input_template[
            "icl_comment"
        ] = f"""Predict the magnitude for [Num] in the comment: COSTCO WHOLESALE <COST.O> UP [Num] PCT IN PREMARKET TRADING AFTER  RESULTS
magnitude: 0
...
magnitude: """
        ...
        self.input_template[
            "icl_headline"
        ] = f"""Predict the magnitude for [Num] in the headline: America in command of Interliga Group A after 3-[Num] win over Atlante
magnitude: 0
...
magnitude: """
    ...
    def load_qnli_template(self):
        self.input_template[
            "icl"
        ] = f"""Based on the options, predict the relationship between the following two statements.
