...
def remove_key_json(json_data, key_to_remove):
    return [
        {key: value for key, value in data.items() if key not in key_to_remove}
...
def trans_to_dict_qp(data):
    data = remove_key_json(
        data,
        [
            "length",
            "offset",
            "comment_sci_10E_char",
            "comment_sci_10E",
            "title_sci_10E_char",
            "title_sci_10E",
            "UNIQUE_STORY_INDEX",
    ...
    for item in data:
        for key in keys:
            ...
            if key == "number":
                ...
                key == "magnitude" or key == "offset" or key == "id" or key == "length"
            ):
                val = int(val)
            else:
                val = str(val)
...
def trans_to_dict_qqa(data):
    data = remove_key_json(
        data, ["type", "question_sci_10E", "question_sci_10E_char", "question_mask"]
    )
